...
def balance_dataset(df, target_column):
    print("Balancing dataset by selecting equal samples per class")

    # Count instances per class
    class_counts = df[target_column].value_counts()

    # Identify majority and minority classes
    majority_class = class_counts.idxmax()  # Class with most samples
    minority_classes = class_counts.index[class_counts.index != majority_class]

    # Keep all instances of minority classes
    minority_samples = df[df[target_column].isin(minority_classes)]
    total_minority_instances = len(minority_samples)

    # Sample the same number of majority instances as total minority instances
    majority_sample = df[df[target_column] == majority_class].sample(n=total_minority_instances, random_state=42)

    # Combine majority and minority samples
    balanced_df = pd.concat([majority_sample, minority_samples])
