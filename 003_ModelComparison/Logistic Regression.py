...
def train_model_logistic_regression(df, target_column, label_mappings, label_encoders):
    """
    Function to train a Logistic Regression model. Steps:
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=FutureWarning)
        lr = LogisticRegression(multi_class="ovr", random_state=42)  # One-vs-Rest for multi-class
    ...
    # Save to a file
    joblib.dump(model_data, '../005_UserUI/trained_logistic_regression_model.pkl')
    print(Fore.LIGHTGREEN_EX + "Model and preprocessing objects saved to 'trained_logistic_regression_model.pkl'." + Style.RESET_ALL)
...
def main():
    ...
    model, X_train_scaled, X_test_scaled, y_train, y_test = train_model_logistic_regression(df, target_column, label_mappings, label_encoders)
